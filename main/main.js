...
function headerNewButton(
  href = "#",
  HTMLclassName = "btn btn-primary",
  innerText = "new URL BUTTON"
) {
  let header = document.querySelector(
    "div.header-container ul.nav-left #toolbox"
  );
...
function headerNewButtonEmbeded(
  href = "#",
  HTMLclassName = "btn btn-primary",
  innerText = "new embeded BUTTON"
) {
  let header = document.querySelector(
    "div.header-container ul.nav-left #toolbox"
  );
...
function 광고판이동() {
  let 광고판HTML = `<button type="button" class="modal-btn btn btn-eie-notice-list" data-toggle="modal" data-target=".modal-view" data-modal-title="알림" data-modal-url="/notice/"><span>본사공지사항</span></button>`;

  let 광고판 = document.querySelector(
    "div.header-container .nav-left .search-box"
  );
...
const renderMain = () => {
  document
    .querySelector("div.header-container ul.nav-left li")
    .insertAdjacentHTML("afterend", `<span id="toolbox"></span>`);
  headerNewButton(
    "https://www.jlive.co.kr/",
    "btn btn-primary",
    "JLive LMS(Presentation Video)"
  );
  headerNewButton(
    "https://www.youtube.com/channel/UCTF7qRuacBq0Eic_en4FYHw",
    "btn btn-danger",
    "EiE Presentation Server"
  );
  headerNewButtonEmbeded("/cletter/", "btn btn-info", "알림장");
  headerNewButtonEmbeded("/community/advice_list", "btn btn-info", "상담일지");
